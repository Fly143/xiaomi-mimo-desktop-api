...
async def chat_completions(
    ...
    # 思考开关：客户端给了 reasoning_effort（low/medium/high）则透传档位
...
async def desktop_import(request: Request, username: str = Depends(verify_admin)):
    ...
    if not fields["mimo_pass_token"]:
        return {"ok": False, "error": "Need mimoPassToken from Desktop cookie store"}

    return await _validate_and_save(
        fields["mimo_pass_token"],
        fields["mimo_user_id"],
        fields["mimo_c_user_id"],
        fields.get("uid") or "",
    )
...
async def _validate_and_save(
    ...
    uid = uid or mimo_user_id or ""

    existing = False
    for i, acc in enumerate(config_manager.config.mimo_accounts):
        if uid and (acc.uid == uid or acc.mimo_user_id == uid):
            config_manager.config.mimo_accounts[i] = MimoAccount(
                mimo_pass_token=mimo_pass_token or acc.mimo_pass_token,
                mimo_user_id=mimo_user_id or acc.mimo_user_id,
                mimo_c_user_id=mimo_c_user_id or acc.mimo_c_user_id,
                uid=uid or acc.uid,
                login_time=now,
                is_valid=True,
            )
    ...
    if not existing:
        config_manager.config.mimo_accounts.append(MimoAccount(
            mimo_pass_token=mimo_pass_token,
            mimo_user_id=mimo_user_id,
            mimo_c_user_id=mimo_c_user_id,
            uid=uid,
            login_time=now,
            is_valid=True,
        ))
    ...
    acc = config_manager.config.mimo_accounts[-1] if not existing else next(
        a for a in config_manager.config.mimo_accounts if a.uid == uid or a.mimo_user_id == uid
    )
